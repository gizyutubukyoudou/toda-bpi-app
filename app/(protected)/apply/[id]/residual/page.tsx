...
export default function ResidualCheckPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!data || data.status !== "completion_reported") { router.replace("/dashboard"); return; }
      setApp(data);
      setFetching(false);
    })();
  ...
  async function handleSubmit() {
    ...
    if (!checkTime.trim()) { setError("残火確認時刻を入力してください"); return; }
    setSubmitting(true);
    setError(null);
    try {
      const res = await fetch(`/api/applications/${id}/residual-check`, {
        method: "POST",
        headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
        body: JSON.stringify({ residualCheckTime: checkTime }),
      });
  ...
  if (fetching || !app) {
    return (
      <>
        <AppHeader title="残火確認" showBack backHref="/dashboard" />
  ...
  return (
    <>
      <AppHeader title="⑥ 残火確認（担当職員）" showBack backHref={`/apply/${id}`} />
      <main className="max-w-2xl mx-auto pb-32 px-4 pt-4 space-y-4">
        ...
        {/* 作業実績 */}
        {(app.wpInspectionTime || app.wpResidualCheckTime || app.completionTime) && (
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div className="px-4 py-2.5 bg-green-50 border-b border-gray-200">
              <h3 className="text-sm font-semibold text-green-700">作業実績</h3>
            </div>
            <div className="p-4 space-y-2">
              ...
              {app.wpResidualCheckTime && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">残火確認時刻（中断時）</span>
                  <span className="font-medium text-gray-800">{app.wpResidualCheckTime}</span>
                </div>
              )}
              {app.completionTime && (
                <div className="flex justify-between text-sm border-t border-gray-100 pt-2 mt-2">
                  <span className="text-gray-500">作業終了時刻（火元責任者報告）</span>
                  <span className="font-medium text-gray-800">{app.completionTime}</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="px-4 py-2.5 bg-primary-50 border-b border-gray-200">
            <h3 className="text-sm font-semibold text-primary-700">最終残火確認（担当職員）</h3>
          </div>
          <div className="p-4 space-y-2">
            <label className="block text-sm text-gray-600">
              残火確認時刻<span className="text-red-500 ml-0.5">*</span>
            </label>
            <TimeSelect value={checkTime} onChange={setCheckTime} />
          </div>
        </div>
